...
{if $diff.old_ver > 0}
        ...
        {if $is_logged}
        <form class='inline' id='form_revert_t{$diff.old_ver}' method='post' action='{$web_prefix}/revert.php?dict_rev={$diff.old_ver}'><button onclick="submit_with_readonly_check(byid('form_revert_t{$diff.old_ver}'))">Вернуть эту версию</button></form>
        {/if}
