...
function get_available_tasks($user_id, $only_editable=false, $limit=0, $random=false) {
    ...
    $type_has_finished_pools = array();
    $res = sql_query("SELECT `type_id`, `doc_link`, `complexity`
                , (SELECT COUNT(*) AS `pool_count` FROM `morph_annot_pools` AS `p` 
                        WHERE `p`.`pool_type` = `morph_annot_pool_types`.`type_id` AND `p`.`status` = ".MA_POOLS_STATUS_ARCHIVED.") AS `archived_pools_count`
    ...
    while ($r = sql_fetch_array($res)) {
        ...
        if ($r['archived_pools_count'] > 0)
            $type_has_finished_pools[] = $r['type_id'];
    }
    ...
    if (!$random)
        foreach ($tasks as $group_id => $v) {
            ...
            $tasks[$group_id]['has_samples'] = in_array($group_id, $type_has_finished_pools);
