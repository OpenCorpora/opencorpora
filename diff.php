...
print "<h3><a href='sentence.php?id=$sent_id'>Предложение $sent_id</a>, изменил ".$r['user_name'].strftime(" %d.%m.%Y в %H:%M", $r['timestamp']).'</h3>';
...
while($r = sql_fetch_array($res)) {
    ...
    if ($r2 && $r1['set_id']==$set_id) {
        print "<tr><th colspan='2'>".$r['pos']."</tr><tr><td valign='top'><b>(Было)<br/>Версия ".$r2['rev_id']." (".($r2['user_id']?$r2['user_name']:'Робот').", ".strftime("%d.%m.%Y, %H:%M", $r2['timestamp']).")</b><pre>".htmlspecialchars(format_xml($r2['rev_text']))."</pre>";
        print "<td valign='top'><b>(Стало)<br/>Версия ".$r1['rev_id']." (".($r1['user_id']?$r1['user_name']:'Робот').", ".strftime("%d.%m.%Y, %H:%M", $r1['timestamp']).")</b><pre>".htmlspecialchars(format_xml($r1['rev_text']))."</pre></tr>";
