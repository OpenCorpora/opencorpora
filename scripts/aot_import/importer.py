...
def do_import(editor, stream):
    ...
    for line in stream:
        ...
        if line.isdigit() or not line:
            ...
            gram_lemma, gram_form = gram.encode('utf-8').split(' ')
            if not in_lemma:
                if new_lemma is not None:
                    new_lemma.save()
                    new_lemma = None
                found = editor.find_lexeme_by_lemma(form_text, ['NOUN'])
                if len(found) > 0:
                    print("found {} {}: {}".format(len(found), form_text, ','.join(found[0].lemma['gram'])))
                    skip = True
                else:
                    new_lemma = Lexeme(lemma=form_text, editor=editor)
                    new_lemma.add_lemma_gram(gram_lemma.split(','))
                    new_lemma.add_form(form_text, gram_form.split(','))
                    skip = False
                in_lemma = True
            elif not skip:
                new_lemma.add_form(form_text, gram_form.split(','))
